...
sum(dirtime) #14.8862 seconds

#trsift = readRDS('trsift.rds')
trsift =
siftime = c()
...
sum(siftime) / 3600 #7.351146 hours
saveRDS(trsift, file = 'trsift4000.rds')
...
sum(dirtime) #3.465966 seconds

#tssift = readRDS('tssift.rds')
...
sum(siftime2) / 3600 #1.805345 hours
saveRDS(tssift, file = 'tssift1000.rds')
...
tsidsift = tsidsift[-e]
tssift = tssift[-(which(sapply(tssift, is.null), arr.ind = TRUE))]
#saveRDS(tsidsift, file = 'tsidsift.rds')
#saveRDS(tssift, file = 'cleantssift')


set.seed(42)

#saveRDS(trsift, file = 'cleantrsift4000.rds')
#saveRDS(tssift, file = 'cleantssift1000.rds')
#saveRDS(tridsift, file = 'cleantridsift4000.rds')
#saveRDS(tsidsift, file = 'cleantsidsift1000.rds')

trsift = readRDS('cleantrsift4000.rds')
tssift = readRDS('cleantssift1000.rds')
tridsift = readRDS('cleantridsift4000.rds')
tsidsift = readRDS('cleantsidsift1000.rds')

coor = vector(mode = 'list', length = length(trsift)) #3634
...
for (i in 1:length(coor)){
    if (nrow(coor[[i]]) < 40){ #delete images with less keypoints --> 10/5 lose 296, 15/6 lose 640, 15/7 lose 640, 17/8 lose 801, 18/9 lose 986, 24/10 lose 1451, 27/11 lose 1729, 27/12, 27/13, 29/14 lose 1921, 34/15 lose 2326, 34/16, 37/17 lose 2547, 37/18, 40/19 lose 2721
        e[k] = i
        k = k + 1
    }
}
...
for (i in 1:length(coor)){
    t1 = Sys.time()
    kms[[i]] = kmeans(coor[[i]], centers = 19, iter.max = 1000000000)
    t2 = Sys.time()
    ktime[i] = t2 - t1
}
sum(ktime) #k5 <10 12.93154 min 5.537457, k6 <15 5.51902 min 15.47092, k7 <15 10.08826 min 0.9862735 s, k8 <17 11.77146 min 49.74899 k9 <19 17.6792 min 15.82605, k10 <24 36.52964 min 39.62689, k11 <27 31.54447 min 54.12041, k12 <27 48.72509 min 22.33233, k13 <27 64.76997 min 80.28653, k14 <29 59.2405 min 100.2406, k15 <34 59.18468 min 30.25336, k16 <34 30.04086 min 80.22792, k17 <37 171.1612 min 87.40172, k18 <37 112.2989 min 68.48628, k19 <40 92.61218 min
...
t2 = Sys.time()
svmtime = t2 - t1 #5 3.18572 s 3.102915, 6 2.628968 s 2.622638, 7 2.768568 s 2.573682, 8 2.601016 s 2.601143, 9 2.411548 s 2.388612, 10 1.706262 s 1.689482, 11 1.393272 s 1.373242, 12 1.433023 s 1.408234, 13 1.639589 s 1.446928, 14 1.254642 s 1.214753, 15 0.7181048 s 0.7207901, 16 0.729248 0.7314091, 17 0.5326009 0.5306149, 18 0.5850179 s 0.541579, 19 0.4025269

predSVM = predict(trmod)
confusionMatrix(predSVM, y_train) #5 17.65% 17.53%, 6 20.41% 20.17%, 7 22.71% 26.51%, 8 24.21% 26.26%, 9 27.11% 26.51%, 10 30.23% 32.07%, 11 33.07% 31.6%, 12 34.96% 34.33%, 13 35.64% 36.48%, 14 39.11% 39.93%, 15 41.44% 42.66%, 16 45.34% 42.66%, 17 45.26% 43.05%, 18 41.95% 48.85%, 19 45.13%

saveRDS(trmod, file = 'trainmodel.rds')
...
confusionMatrix(predSVM2, y_test) #18 9.2%

#PCA
#pcatime = c()
#for (i in 1:length(coor)){
#  t1 = Sys.time()
#  p = prcomp(coor[[i]], center = TRUE, scale = TRUE)
#  t2 = Sys.time()
#  pcatime[i] = t2 - t1
#}
#sum(pcatime)
